...
def convert_file(path_interview_file, path_csv_file, path_result_file = None):
    ...
    for row in list_rows:
        if '   (Super)' in row and flag:
            ...
            if len(test_key) > 1:
                ...
                for key in test_key:
                    if 'e' not in key:
                        key = key + 'e'
                    key_interview += key  + '/'
                key_interview = find_between(key_interview[:-1], '', '.').strip()
            else:
            '''
            ...
            '''
            if 'e' not in key_interview:
                key_interview = key_interview + 'e'
            '''
    ...
    for key_interview in interview_dict.keys():
        for key in key_interview.split('/'):
            value_write = ''
            value_write_vazio = ''
            if csv_values_dict.has_key(key):
                for csv_list in csv_values_dict.get(key):
                    ...
                    if (csv_list[0] + 1) == len(csv_header_list):
                        ...
                        header.update({key : '****' + value_write.decode("latin1").encode("utf8") + '\n'})
